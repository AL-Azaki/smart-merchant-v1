...
class CashManagementPostingIntegrationService
{
    ...
    public function handleCashManagementPostingRequest(PostingRequestDTO $request, string $userId): void
    {
        $data = [
            'business_id' => $request->businessId,
            'fiscal_year_id' => Str::uuid()->toString(),
            'fiscal_period_id' => Str::uuid()->toString(),
            'journal_number' => 'CSH-' . strtoupper(Str::random(6)),
            'document_date' => $request->date,
            'journal_type' => 'CashTransaction',
