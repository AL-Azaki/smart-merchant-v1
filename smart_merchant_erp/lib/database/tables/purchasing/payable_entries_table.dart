...
/// Drift table definition for `payable_entries`.
///
...
@DataClassName('PayableEntry')
class PayableEntries extends Table {
  @override
  String get tableName => 'payable_entries';
  ...
  /// Composite Foreign Key linking to `supplier_payables(business_id, id)` (CASCADE).
  TextColumn get supplierPayableId => text().named('supplier_payable_id')();
  ...
  /// Timestamp when this payable entry/allocation took place (`CURRENT_TIMESTAMP`).
  ...
  /// Type of payable entry (`Payment`, `Adjustment`, `WriteOff`).
  ...
  /// Identifier of the device where the payable entry record was created or modified (`device_id`).
  ...
  List<String> get customConstraints => [
    'FOREIGN KEY (business_id, supplier_payable_id) REFERENCES supplier_payables(business_id, id) ON DELETE CASCADE',
