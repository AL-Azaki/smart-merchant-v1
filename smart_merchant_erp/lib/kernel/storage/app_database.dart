...
import '../../database/tables/inventory/stock_counts_table.dart';
import '../../database/tables/inventory/stock_count_items_table.dart';
import '../../database/converters/stock_count_status_converter.dart';
...
import '../../database/tables/system/archive_documents_table.dart';
  ...
  MigrationStrategy get migration {
    return MigrationStrategy(
      onUpgrade: (m, from, to) async {
        if (from < 2) {
          await m.createTable(archiveDocuments);
        }
        if (from < 3) {
        ...
        if (from < 4) {
          await m.addColumn(products, products.currencyId);
        }
        if (from < 5) {
          await m.addColumn(products, products.showInStore);
        }
      },
